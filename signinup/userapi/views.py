...
class UserDetailViewSet(APIView):
    ...
    def get(self, request, format=None):
        ...
        if 'TOKEN' in self.request.COOKIES:
            ...
            user_no = jwt.decode(token, SECRET_KEY, algorithm='HS256')
            print(user_no)
            user = User.objects.get(user_no=user_no['user'])
            print(user)
            return JsonResponse({'TOKEN':token, 'user':user_no,'user_email':user.user_email,'user_nickname':user.user_nickname,'user_gender':user.user_gender,'user_birthdate':user.user_birthdate}, status = 200)
